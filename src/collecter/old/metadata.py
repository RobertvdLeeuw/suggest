...
if os.getenv("TEST_MODE"):
    ...
    LOGGER.info("Importing mock APIs.")

    mb = musicbrainz_fake()
    pylast = pylast_fake()
    spotipy = spotipy_fake()
else:
    LOGGER.info("Importing (actual) APIs.")
...
from models import Artist, EmbeddingAuditus, EmbeddingJukeMIR
...
def _sp_to_mb(artist_id: str) -> str | None:
    ...
    if len(res["recording-list"]) == 0:
        LOGGER.info(
            f"No MusicBrainz artists found in response for '{artist_name} - {top_song}': {res}"
        )
        return
    ...
    for mb_artist in res["recording-list"][0]["artist-credit"]:
        ...
        artist_names = [
            a["artist"]["name"]
            for a in res["recording-list"][0]["artist-credit"]
            if isinstance(a, dict) and "artist" not in a
        ]
        LOGGER.warning(
            f"No MusicBrainz match found for {artist_name} among: {', '.join(artist_names)}"
        )
        return
...
def get_similar_artists(spotify_artist_id: str, degrees=1) -> list[str]:
    ...
    if lfm_artist is None:
        return []

    similar = [_lastfm_to_sp(a.item) for a in lfm_artist.get_similar(limit=3)]
...
async def check_in_table(table, col, value, session=None):
    ...
    LOGGER.debug(
        f"Checking {table.__name__} for {col.name}={value}: {'Found' if item else 'Not found'}"
    )
    return item
...
QUEUE_OBJECTS = [
    (EmbeddingJukeMIR, QueueJukeMIR, "JukeMIR"),
...
async def queue_sp_user():  # TODO: Take userID instead of current user.
    ...
    playlist_tracks = [track for p in playlist_ids for track in _get_sp_playlist_tracks(p)]

    await _add_to_db_queue(liked + playlist_tracks)


async def _queue_sp_history_chunk(history: list[dict]):
...
async def queue_sp_history(folder="/home/robert/Downloads/Spotify Extended Streaming History"):
    for file in os.listdir(folder):
        if not file.endswith(".json"):
            continue

        with open(f"{folder}/{file}", "r") as f:
            await _queue_sp_history_chunk(json.load(f))
...
async def simple_queue_new_music():
    ...
    async with get_session() as s:
        artists = await s.execute(select(Artist.spotify_id).order_by(func.random()).limit(10))
...
async def create_push_artist(spotify_id: str, session=None) -> Artist | None:
    ...
    artist_result = await session.execute(select(Artist).where(Artist.spotify_id == spotify_id))
    if artist := artist_result.scalar_one_or_none():
        return artist

    try:
        stmt = insert(Artist).values(spotify_id=spotify_id, artist_name=artist_name)
        ...
        if result.rowcount > 0:
            ...
            if lfm_artist := _sp_to_lastfm(spotify_id):
                ...
                try:
                    ...
                    LOGGER.warning(
                        f"Received invalid LastFM artist for '{artist_name}': {lfm_artist}"
                    )
...
async def create_push_track(spotify_id: str, session=None) -> Song:
    ...
    try:
        song = Song(spotify_id=spotify_id, song_name=track["name"], artists=artists)

        metadata = []
        try:
            if lfm_song := lastfm.get_track(
                artist=track["artists"][0]["name"], title=track["name"]
            ):
        ...
        try:
            if mb_songs := mb.search_recordings(
                f'artist:"{track["artists"][0]["name"]}" AND recording:"{track["name"]}"'
            ):
                LOGGER.debug(f"Adding MusicBrainz tags to track '{track['name']}'.")
                if mb_songs.get("recording-list"):
                    tags_data = mb.get_recording_by_id(
                        mb_songs["recording-list"][0]["id"], includes=["tags", "user-tags"]
                    )
                    metadata.extend(
                        [
                            SongMetadata(
                                type=MetadataType.genre, value=tag["name"], source="MusicBrainz"
                            )
            ...
            LOGGER.debug(
                f"Failed to get MusicBrainz data for {track['name']}: {traceback.format_exc()}"
            )
        ...
        song_result = await session.execute(select(Song).where(Song.spotify_id == spotify_id))
...
async def _add_song_listens(user_id: int, tracks: list[dict], session=None):
    ...
    for track in tracks:
        ...
        missing_required = [
            required for required in ["spotify_id", "ms_played"] if required not in track
        ]
        if missing_required:
            LOGGER.warning(
...
async def push_sp_user_to_db(spotify_id=None, session=None) -> User:
    LOGGER.info(
        f"Adding user '{spotify_id}' to DB." if spotify_id else "Adding current Spotify user to DB."
    )

    try:
        sp_user = sp.current_user()
        LOGGER.debug(
            f"Retrieved Spotify user: {sp_user.get('display_name', 'Unknown')} "
    ...
    try:
        stmt = insert(User).values(spotify_id=sp_user["id"], username=sp_user["display_name"])
        stmt = stmt.on_conflict_do_nothing(index_elements=["spotify_id"])
        result = await session.execute(stmt)
        ...
        if result.rowcount > 0:
            ...
            LOGGER.debug(
                f"User {sp_user['display_name']} ({sp_user['id']}) already exists in database"
            )

        user_result = await session.execute(select(User).where(User.spotify_id == sp_user["id"]))
...
async def add_recent_listen_loop(user_spotify_id: str):
    ...
    while True:
        try:
            ...
            if (
                not new_listen
                or not new_listen.get("item")
                or not new_listen["is_playing"]
                or new_listen["item"]["type"] != "track"
            ):
            ...
            if current_listen["item"]["id"] == new_listen["item"]["id"]:
                if new_listen["progress_ms"] < current_listen["progress_ms"]:
                    LOGGER.debug(
                        f"Song rewound from {current_listen['progress_ms']}ms "
                        f"to {new_listen['progress_ms']}ms"
                    )

                    listen_chunks.append(
                        {"from_ms": latest_chunk_start, "to_ms": current_listen["progress_ms"]}
                    )
                    ...
                    new_listen["progress_ms"] - current_listen["progress_ms"] > SLEEP_TIME_S * 5000
                ):
                    LOGGER.debug(
                        f"Song fast-forwarded from {current_listen['progress_ms']}ms to {new_listen['progress_ms']}ms"
                    )
            ...
            LOGGER.debug(
                f"Song changed from '{current_listen['item']['name']}' "
            ...
            LOGGER.debug(
                f"Saving listen: {ms_played}ms of '{current_listen['item']['name']}' "
