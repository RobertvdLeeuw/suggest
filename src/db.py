...
from sqlalchemy import URL, event, select, text
...
class DatabaseManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            async with self._engine.begin() as conn:
                ...
                LOGGER.info(
                    f"Database connection test to '{url.database}' successful (PID: {os.getpid()})"
                )
            ...
            LOGGER.info(
                f"Database engine and session factory initialized successfully (PID: {os.getpid()})"
            )

        except Exception as e:
            LOGGER.error(
                f"Could not initialize database (PID: {os.getpid()}): {traceback.format_exc()}"
            )
            await self.cleanup()
    ...
    async def get_session(self) -> AsyncGenerator[AsyncSession, None]:
        ...
        try:
            ...
            LOGGER.error(
                f"Session error, rolling back (PID: {os.getpid()}): {traceback.format_exc()}"
            )
            raise
        finally:
            await session.close()
...
async def get_embeddings(emb_type) -> np.ndarray:  # Structured array (basically a better dict.)
    ...
    dtype = [("song_id", "i4"), ("chunk_id", "i4"), ("embedding", f"f4", (embedding_dim,))]

    data = np.fromiter(((x.song_id, x.chunk_id, x.embedding) for x in items), dtype=dtype)
