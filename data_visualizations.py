...
def _():
    ...
    T = 2000
    return EMBEDDINGS, LinUCB, T, mo, pd, reduced
...
def _(LinUCB, T, mo, reduced):
    ...
    random_rewards = list(reduced[reduced["chunk"] == '0:00 - 0:24'].score.apply(
        lambda s: float(s in ["liked", "loved"])
    ).sample(frac=1))

    from itertools import accumulate
    from operator import sub

    random_goods = GoodSongsLeft()
    random_goods.values = list(accumulate(random_rewards, sub, initial=len(reduced[(reduced["chunk"] == '0:00 - 0:24') & (reduced["score"].isin(["liked", "loved"]))].index)))


    t_random = Trajectory(model_name="Random", T=T, metrics=[GoodSongsLeft])
    t_random.metrics = {random_goods.name: random_goods}

    from models.linear import Lin, LinEps

    t_eps = LinEps(n_dim=250, 
                   alpha=0.5, 
                   update_step=25, 
                   feature_start=-1, 
    ...
    return GoodSongsLeft, SongValuesUCB, SongsPicked, plot_all, t


@app.cell(hide_code=True)
def _(GoodSongsLeft, mo, plot_all, t):
